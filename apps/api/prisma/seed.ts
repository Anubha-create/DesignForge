...
async function main() {
  ...
  // 3. Ensure a clean slate: clear any pre-existing demo attempts and submissions
  // so candidates read the problem statement and design classes and attributes from scratch.
  await prisma.evaluationCriterionResult.deleteMany();
  await prisma.evaluation.deleteMany();
  await prisma.submission.deleteMany();
  await prisma.designSnapshot.deleteMany();
  await prisma.changeTestResult.deleteMany();
  await prisma.attempt.deleteMany();
  console.log('Clean slate ready: All problems are untouched. Candidates design classes and attributes from scratch.');
